...
public class Ocl {
    ...
    public static <D, R> Map<D, R> excludeAllMap(Map<D, R> m1, Map m2) { // m1 - m2
    ...
    public static <D, R> Map<D, R> excludingMapKey(Map<D, R> m, D k) { // m - { k |-> m(k) }
        Map<D, R> res = new HashMap<D, R>();
        res.putAll(m);
        res.remove(k);
        return res;
    ...
    public static <D, R> Map<D, R> excludingMapValue(Map<D, R> m, R v) { // m - { k |-> v }
        Map<D, R> res = new HashMap<D, R>();
        Set<D> keys = m.keySet();
